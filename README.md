...
## Python 断点调试（Windows / Linux）

使用 nvim-dap 和 debugpy。先在项目的 Python 环境中执行 `python -m pip install debugpy`，
然后进入项目根目录启动 Neovim。无需在仓库中填写个人绝对路径。

按 Ctrl+5（备用：空格 d c） 启动时确认 Python 路径；默认依次查找已激活的 virtualenv / Conda 环境、
当前目录的 `.venv`、PATH 中的 Python。调试器和程序使用同一个解释器。
工作目录为 Neovim 当前目录，因此应从项目根目录启动。

| 按键 | 功能 |
|---|---|
| Ctrl+9（备用：空格 d b） | 设置/取消断点 |
| Ctrl+5（备用：空格 d c） | 启动/继续 |
| Ctrl+0（备用：空格 d n） | 执行下一行，不进入函数 |
| Ctrl+-（备用：空格 d i） | 进入函数 |
| Ctrl+=（备用：空格 d o） | 执行到当前函数返回 |
| 空格 d r | 打开/关闭调试交互窗口 |
| 空格 d v | 查看光标处变量 |
| 空格 d q | 结束调试 |
| 空格 d x | 清空所有断点（不结束调试） |

在可执行代码行按 Ctrl+9（备用：空格 d b），再按 Ctrl+5（备用：空格 d c）；停住后打开交互窗口，输入变量名或表达式。
断点行尚未执行，查看变量时应确认它已在之前的代码中赋值。
Ctrl+数字/符号能否使用取决于终端支持及其快捷键设置；没有反应时使用备用键位。所有调试键位在普通模式下使用。
